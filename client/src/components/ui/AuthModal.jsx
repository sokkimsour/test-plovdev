...
export default function AuthModal({ isOpen, onClose }) {
  ...
  const [formData, setFormData] = useState({
    fullName: '', // Change username to fullName
    email: '',
    password: '',
    gender: 'other', // Add gender with a default value
  });
  ...
  const handleVerifyOtp = async () => {
    setLoading(true);
    try {
      // Join the array of 4 digits into a single string
      const fullOtp = otpCode.join('');
      const backendURL = 'http://localhost:5000';

      // Send both the code and the userId (Required by verifyForgotOtp)
      ...
      setActivePanel('signin'); // Go back to login
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      ...
      <div className="bg-gray-900/60 backdrop-blur-xl rounded-3xl p-8 w-full max-w-md shadow-[0_8px_30px_rgb(0,0,0,0.3)] border border-gray-700/50 relative animate-fade-in-up text-white">
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="email" placeholder="Enter your email" name='email' value={formData.email} onChange={handleChange} className="w-full px-4 py-3.5 rounded-xl bg-gray-800/50 border border-gray-700 focus:border-[#00D4FF] focus:ring-1 focus:ring-[#00D4FF] outline-none text-sm font-medium text-white placeholder-gray-500 transition-all" />
                </div>
                <div>
                  ...
                  <input type="password" placeholder="Enter your password" name='password' value={formData.password} onChange={handleChange} className="w-full px-4 py-3.5 rounded-xl bg-gray-800/50 border border-gray-700 focus:border-[#00D4FF] focus:ring-1 focus:ring-[#00D4FF] outline-none text-sm font-medium text-white placeholder-gray-500 transition-all" />
                </div>
              </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" placeholder="Enter your full name" name='fullName' value={formData.fullName} onChange={handleChange} className="w-full px-4 py-3.5 rounded-xl bg-gray-800/50 border border-gray-700 focus:border-[#00D4FF] focus:ring-1 focus:ring-[#00D4FF] outline-none text-sm font-medium text-white placeholder-gray-500 transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-300 mb-1">Email</label>
                  <input type="email" placeholder="Enter your email" name='email' value={formData.email} onChange={handleChange} className="w-full px-4 py-3.5 rounded-xl bg-gray-800/50 border border-gray-700 focus:border-[#00D4FF] focus:ring-1 focus:ring-[#00D4FF] outline-none text-sm font-medium text-white placeholder-gray-500 transition-all" />
                ...
                <div>
                  ...
                  <input type="password" placeholder="Create a password" name='password' value={formData.password} onChange={handleChange} className="w-full px-4 py-3.5 rounded-xl bg-gray-800/50 border border-gray-700 focus:border-[#00D4FF] focus:ring-1 focus:ring-[#00D4FF] outline-none text-sm font-medium text-white placeholder-gray-500 transition-all" />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3.5 rounded-xl bg-gray-800/50 border border-gray-700 focus:border-[#00D4FF] focus:ring-1 focus:ring-[#00D4FF] outline-none text-sm font-medium text-white transition-all"
                  >
        ...
        {/* 4. OTP PANEL */}
        {/* Simplified OTP PANEL */}
        {activePanel === 'otp' && (
          <div>
            <button className="text-sm font-bold text-gray-400 mb-6 flex items-center hover:text-white transition-colors" onClick={() => setActivePanel('signup')}>
              <ArrowLeftIcon className="w-4 h-4 mr-1.5" /> Back
            </button>
            ...
            <div className="mb-8">
              {/* Replaced 4 boxes with 1 input for easier state management */}
              <input
                type="text"
                maxLength="4"
                placeholder="1234"
                value={otpCode[0]} // Temporarily storing the whole code in index 0
                onChange={(e) => setOtpCode([e.target.value, '', '', ''])}
                className="w-full text-center text-2xl font-bold py-4 rounded-xl bg-gray-800/50 border border-gray-700 focus:border-[#00D4FF] focus:ring-1 focus:ring-[#00D4FF] outline-none text-white shadow-sm transition-all tracking-[1em]"
              />
            </div>
            <button
              className="w-full bg-[#00D4FF] text-black font-bold py-3.5 rounded-xl hover:opacity-90 transition shadow-sm"
              onClick={handleVerifyOtp} // Call the new function!
            >
